...
# lists for month and day to check user input validity later
month_choice = ['january', 'february', 'march', 'april', 'may', 'june', 'all']
day_choice = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'all']


# function to validate user input
def check_user_input(user_input, input_type):
    """
    This is to validate and cross-reference list to the entries the user provides
    in case user inputs incorrect input or invalid, program won't crash and will loop.
    will keep looping till correct input provided
    :param user_input: takes city, day, and month as entries
    :param input_type: cross-reference with list
    ...
    while True:
        ...
        try:
            if input_user_entered in ['chicago', 'new york city', 'washington'] and input_type == 'c':
                break
            elif input_user_entered in month_choice and input_type == 'm':
                break
            elif input_user_entered in day_choice and input_type == 'd':
                break
            else:
                if input_type == 'c':
...
def get_filters():
    """
    This will be the beginning of our script and it's used to receive and store the entries.
    :return: The choices user selects.
    """
    print('We\'ve got a variety of filters to choose from, hop right in.')
    ...
    month = check_user_input(
        "For filtering data by specific mounth please enter month name from (january, february, march, april, may, june) otherwise enter 'all'\n",
        'm')
    # get user input for day of week (all, monday, tuesday, ... sunday), and check user input validity
    day = check_user_input(
        "For filtering data by specific day please enter day name from (sunday, monday, tuesday, wednesday, thursday, friday, saturday) otherwise enter 'all'\n",
        'd')
...
def greet(name):
    try:
        ...
        print("Please make sure you enter only letters, as ", e)
greet(input("Greetings, May I have your name?\n"))
...
def show_row_data(df):
    """
    This is to provide user with the raw data.
    :param df: the file containing raw data
    :return: the raw data
    """
    row = 0
    while True:
...
print("Thanks for your time, come again.")
